...
function HighlightedSentence({ text, className = '' }: { text: string; className?: string }) {
    ...
    <span className={className}>
      {parts.map((seg, i) =>
        seg.highlighted ? (
          <span
            key={i}
            className="bg-indigo-100 text-indigo-700 px-1 rounded-md font-bold"
          >
            {seg.part}
          </span>
        ) : (
...
export function SentenceQuiz({ words, onBack, onComplete }: Props) {
  ...
  return (
    <div className="flex flex-col items-center gap-5 px-4 py-6 max-w-lg mx-auto">
      ...
      <div className={`w-full rounded-2xl shadow-sm p-8 text-center transition-colors duration-200 ${
        ...
        {/* Korean sentence with highlighted target word */}
        <div className="flex items-center justify-center gap-2 mb-4">
          <p className="text-2xl font-bold text-gray-900 font-korean leading-relaxed">
            <HighlightedSentence text={current.word.example.korean} />
          </p>
          <SpeakButton text={current.word.korean} />
        </div>

        {/* Hint: show full Japanese sentence */}
        {showHint || phase !== 'question' ? (
          <p className="text-gray-500 text-sm">
            <HighlightedSentence text={current.word.example.japanese} />
          </p>
        ) : (
          <button
            onClick={() => setShowHint(true)}
            className="text-sm text-gray-300 hover:text-indigo-400 underline"
          >
            ヒントを表示
          </button>
        )}
